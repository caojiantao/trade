...
<form id="js-form" enctype="multipart/form-data" method="post">
  <table width="100%" cellspacing="1" cellpadding="0" bgcolor="#CCCCCC">
    ...
    <tr>
      <td>商品详情页logo下方</td>
      <td><textarea class="resizable" name="goodsDetailBottom"
                    style="visibility:hidden;">${website.goodsDetailBottom}</textarea></td>
    </tr>
    <tr>
      <td>商品详情页logo右侧</td>
      <td><textarea class="resizable" name="goodsDetailRight"
                    style="visibility:hidden;">${website.goodsDetailRight}</textarea></td>
    </tr>
    <tr>
      <td>首页图文描述</td>
      <td><textarea class="resizable" name="introduction" style="visibility:hidden;">${website.introduction}</textarea>
      </td>
    </tr>
    <tr>
...
<script type="text/javascript">
    ...
    $('#js-form').submit(function () {
      $(this).ajaxSubmit({
        ...
        success: function (result) {
          $.messager.alert('提示', result.success ? '更新成功！' : result.message, 'info');
        }
      });
      return false;
    });
  ...
  createEditor(['[name="goodsDetailBottom"]', '[name="goodsDetailRight"]', 'textarea[name="introduction"]'])
</script>
</body>
</html>