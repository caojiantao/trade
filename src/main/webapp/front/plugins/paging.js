	...
	$.extend ({
		setPages: function(options) {
			...
			var PAGING = {
				...
				// 页面加载
				initPaging: function() {
					var offset = (this.pIndex - 1) * this.limit;//偏移量
					 ...
					 $.ajax({
					 	...
					 	success: function(data) {
							...
							var total = data.total,
								rows = data.rows;
							    PAGING.pageNum = Math.ceil(total / PAGING.limit);
							PAGING.switchPages();
							opts.onClick(PAGING.pIndex, PAGING.pageNum, total, rows);
					 	}
					 });
				},
				//点击分页按钮
				...
				switchPages: function() {
					...
					if (totalP < this.pIndex) {
						console.log('总页数不能小于当前页数！', totalP, this.pIndex);
						return false;
					}
					//不需要省略
					if (totalP < pageL) {
